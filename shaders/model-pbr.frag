...
void main ()
{
	vec3	base       = texture ( sampler2D ( textures [push.albedo], samp ), tx ).rgb;
	vec3	n          = texture ( sampler2D ( textures [push.normal], samp ), tx ).xyz * 2.0 - vec3 ( 1.0 );
	float	roughness  = texture ( sampler2D ( textures [push.roughness], samp ), tx ).x;
	float	metallness = texture ( sampler2D ( textures [push.metallic], samp ), tx ).x;
...
//color = vec4 ( n2, 1.0 );
//color = vec4 ( l, 1.0 );	// l2, v2 are nan, v.rg are nan

//color = vec4 ( pow ( diff*base, vec3 ( 1.0 / gamma ) ), 1.0 );
