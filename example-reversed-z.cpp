...
public:
	PbrWindow ( int w, int h, const std::string& t, DevicePolicy * p ) : VulkanWindow ( w, h, t, true, p )
	{
...
//		mesh = loadMesh ( device, "../../Models/teapot.3ds", 0.04f );
	...
	virtual	void	createPipelines () override 
	{
		...
		pipeline
				...
				.setDepthTest      ( true )
				.setDepthCompareOp ( VK_COMPARE_OP_GREATER )
				.setDepthWrite     ( true )
				.create            ( renderPass );
	...
	void	createCommandBuffers ( Renderpass& renderPass )			// size - swapChain.framebuffers.size ()
	{
		...
		for ( size_t i = 0; i < commandBuffers.size(); i++ )
		{
			commandBuffers [i]
				.begin             ()
				.beginRenderPass   ( RenderPassInfo ( renderPass ).framebuffer ( framebuffers [i] ).extent ( swapChain.getExtent ().width, swapChain.getExtent ().height ).clearColor ().clearDepthStencil ( 0.0f ) )
				.pipeline          ( pipeline )
	...
	void updateUniformBuffer ( uint32_t currentImage )
	{
		...
		uniformBuffers [currentImage]->proj     = perspectiveProjection ( 45, getAspect (), 0.1f );		// projectionMatrix ( 45, getAspect (), 0.1f, 10.0f );
	...
	glm::mat4 perspectiveProjection ( float fovY, float aspect, float zNear ) const
	{
...
int main ( int argc, const char * argv [] ) 
{
	...
	policy.depthFormat = VK_FORMAT_D32_SFLOAT;

	return PbrWindow ( 800, 600, "Reversed-Z", &policy ).run ();
}
